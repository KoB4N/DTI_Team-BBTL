...
int counter = -1; /////// set 0 for set 1, -1 for set 2

const float alpha = 0.5; // Smoothing factor, adjust as needed
...
class MyAdvertisedDeviceCallbacks : public BLEAdvertisedDeviceCallbacks {
    void onResult(BLEAdvertisedDevice advertisedDevice) {
        ...
        else  { // switch back later 1
        // if (true){
             if (advertisedDevice.getName().find("esp1") != std::string::npos ) { // only check with esp3 table set 2
                ...
                int darkness_level = map(rssi, -60, -30, 10, 1); // Map smoothed RSSI to darkness level (1 to 10)

                // Calculate darker shades
                // Calculate darker shades with boundaries
                int darkerRed = constrain(originalRed * (10 - darkness_level) / 10, 0, 255);
                int darkerGreen = constrain(originalGreen * (10 - darkness_level) / 10, 0, 255);
                int darkerBlue = constrain(originalBlue * (10 - darkness_level) / 10, 0, 255);

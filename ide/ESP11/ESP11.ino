    ...
    void onResult(BLEAdvertisedDevice advertisedDevice) {
        ...
        // if (true){
             if (advertisedDevice.getName().find("esp3") != std::string::npos ) { // only check with esp3 table set 2
                ...
                int darkness_level = map(smoothedRSSI, -60, -30, 10, 1); // Map smoothed RSSI to darkness level (1 to 10)

                // Calculate darker shades
                int darkerRed = constrain(originalRed * (10 - darkness_level) / 10, 5, 255);
                int darkerGreen = constrain(originalGreen * (10 - darkness_level) / 10, 5, 255);
                int darkerBlue = constrain(originalBlue * (10 - darkness_level) / 10, 5, 255);
